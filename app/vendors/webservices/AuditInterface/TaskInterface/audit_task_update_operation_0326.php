...
class AuditTaskUpdateOperation extends Object{
	...
	public function __construct(){
		...
		//mpc操作
		if (IS_MPC_CUSTOM==1){
			App::import('Vendor', 'AuditTaskMpcCntvOperation', array('file' => 'webservices' . DS . 'AuditInterface'. DS . 'TaskInterface' . DS . 'audit_task_mpc_operation_cntv.php'));
	...
	public function updateTaskWithInfo($theParams){
		...
		if ($state==PASS_AUDIT_TASK_STATE){
			...
			if ($isFinal){
				...
				if (!$commitStateMain){
					$commitStateSpare = $this->mpcOperation->commitMpc(-1);
				}

				$reWriteMpcState = $this->mpcOperation->rewriteMpc($taskIDData);
				//:cntv
				if (IS_MPC_CUSTOM==1){
					$this->sdOperateLog($state,$theTime);
				}

				$mpcState = $mpcState && ($commitStateMain || $commitStateSpare) && $reWriteMpcState;
			}
			if ($mpcState){
				//更新元数据
		...
		elseif ($state == RETURN_AUDIT_TASK_STATE){
			...
			$baseInfoState =  $this->_updateCurTaskInfo($theTime,$metaData);
			$updateMetaDataState = $this->_rewriteMetaData($metaData);
			return $baseInfoState && $updateMetaDataState;
		}
	}
	private function baseOperation($theTaskID,$metaData,$theTime){
		//更新基本审核信息
		$updateBaseInfoState = $this->_updateCurTaskInfo($theTime,$metaData);
	...
	private function noticeUpdateState($theTime){
		...
		if (strtoupper($workStationType)==strtoupper(EDIT_MODE_CP)){
	 ...
	 * 更新任务基本信息
	 */
	private function _updateCurTaskInfo($theTime,$metaData=NULL){
		...
		//特殊处理
		if ($metaData){
			extract($metaData);//$newKeyAttributeArray
			...
			if (defined('PGMNAME') && in_array(PGMNAME, $editAttributes)){
				...
				if (IS_CNTV){
					...
					if (isset($newKeyAttributeArray['Title'])){
						$taskInfoData = array_merge($taskInfoData,array('pgmname'=>$newKeyAttributeArray['Title']['Value']));
					}
				}
	...
	private function _updateWorkFlow($workFlowID,$newWorkFlowType){
		...
		//调用接口
		if ($state == RETURN_AUDIT_TASK_STATE){
			$workFlowOperation = new AuditWorkFlowOperation($workFlowID, 'back', $taskType, $auditLevel, $userName,null,null,$auditNote);
		}
		else {
		...
		if ($metaDataUpdate){
			foreach ($metaDataUpdate as $tmpUpdateCode=>$tmpUpdateValue) {
				...
				if (in_array($tmpLogCode, $attributeCode)){
					...
					$tmpCodeItem['Value'] = 'Modify TO['.$this->format_sepcial_chars($tmpUpdateValue).'];Edit by ['.$userName.']';
	 ...
	 * Cntv特殊日志
	 */
	private function sdOperateLog($state,$theTime){
		if ($state == PASS_AUDIT_TASK_STATE){
			$auditResult = 0;
		}else {
			$auditResult = 2;
		...
		$existCount = $operateLog->find('count',array('conditions'=>array('programguid'=>$this->curTaskInfo['pgmguid'],'operatetype'=>3)));
		if ($existCount>0){
			return true;
		}
		$sqlStr = "insert into
					      smm_sdoperatelog(programname,programguid,entityid,pgmlength,operatorname,operatetype,operateresult,operatetime,exchannel,excolumn)   
					      values('".$this->escapeValue($this->curTaskInfo['pgmname'])."',
					             '".$this->escapeValue($this->curTaskInfo['pgmguid'])."',
					             0,
					             ".$this->curTaskInfo['pgmlength'].",
					             '".$this->escapeValue($this->paramTaskInfo['AuditorName'])."',
					             3,
					             $auditResult,
					             ".DbHandle::_getTimestampFormat($theTime).",
					             '".$this->escapeValue($this->curTaskInfo['channelname'])."',
					             '".$this->escapeValue($this->curTaskInfo['columnname'])."'
					      )";
		
		return $operateLog->newSetData(ChangeEncode::changeEncodeFromUTF8($sqlStr));		
	}
	/**
	 * sql值特使处理
	 */
...
//	$str = str_replace('&', '&amp;', $str);
	...
	//反转义XML特殊字符
	protected function de_format_sepcial_chars($str) {
	$str = str_replace('&amp;', '&', $str);
	$str = str_replace('&lt;', '<', $str);
	$str = str_replace('&gt;', '>', $str);
	$str = str_replace("&apos;", "'", $str);
