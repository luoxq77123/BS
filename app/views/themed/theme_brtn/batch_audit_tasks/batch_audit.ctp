			...
			<div class="title">				
				<select name="" onchange="changeFilealias(this.value,<?php echo $theTaskID;?>)">
				    ...
				    ?>	
                                ...
                                正在审核: 
                <span><?php echo $this->ViewOperation->subStringFormat($taskInfo['pgmname'], 0, AUDIT_NAME_NUM);?></span> 
			</div>
			<div class="play">
...
function updataData(key,value){
	if(revisedMetaData){
		if(revisedMetaData.indexOf(key) == -1){
            revisedMetaData += ","+key+"|"+value;
		}
	}
	else{
		revisedMetaData += key+"|"+value;
...
function nrms_edit() {
	$(".nrms_cont").each(function() {
		...
		_this.delegate("div", "dblclick", function(){
			var oval = $(this).text();
			$(this).after('<input type="text" value="'+ oval +'">').remove();
			iptOper();
		})
	})
}
...
function commit(state,taskid){	  
       ...
       datas = datas+"&UpdateMetaData="+revisedMetaData;
