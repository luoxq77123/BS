...
<?php 	function escapeValue($str = NULL){
	$str = str_replace("&apos;&apos;", "'", $str);
	    $str = str_replace('\&quot;', '&quot;', $str);
	    return $str;
	}
		...
		<ul class="of" id="slvList">		    
		    ...
		    <li>
			    <div class="liCont">
			        ...
			        ?>
                    <a href="#" class="p" title="<?php echo escapeValue($pgmData['pgmname']);?>" ondblclick="dbAudit('<?php echo $tmpTaskID?>')">
                        <?php $newPicPath = $this->ViewOperation->getPicPath($pgmData['picpath']);?>
				        <img src="<?php echo $newPicPath;?>" alt="" title="<?php echo escapeValue($pgmData['pgmname']);?>" style="width:169px;height:121px;"/>
                    </a>
                    <a href="#" class="t" title="<?php echo escapeValue($pgmData['pgmname']);?>">
                        ...
                        <?php echo $this->ViewOperation->subStringFormat(escapeValue($pgmData['pgmname']),THUMB_NAME_NUM);?>
                        </a>
                    <span class="checkIpt">
                       <?php if ($auditState == NOT_AUDIT_TASK_STATE) :?>
                       ...
                       <script type="text/javascript">
                           <?php if ($selectedList && in_array($pgmData['taskid'], $selectedList)):?>
                               ...
                               $(checkid).parent().parent().parent('li').addClass("hover").find(".checkIpt").show();
                           <?php endif;?>
                       </script>
                    </span>
                </div>
