 ...
 */
	function value($data, $column = null, $safe = false) {
		...
		if ($column == 'integer' || $column == 'float' || $column == 'timestamp') {
			return $data;
		}
		return "'" . $data . "'";
	}
 ...
 */
	function limit($limit, $offset = null,$order = null) {
		if ($limit) {
			$pre = '';
			$rt = '';
			$rn ='';
			...
			if (!strpos(strtolower($limit), 'limit') || strpos(strtolower($limit), 'limit') === 0) {
				$pre = 'SELECT * FROM ( ';
				$rt = sprintf(' ) WHERE ROWNUMBER >=%d AND ROWNUMBER <=%d  FETCH FIRST %d ROWS ONLY', $offset+1,$limit+$offset,$limit);
			    $rn = ' ,ROW_NUMBER() OVER ('.$order.') AS ROWNUMBER';
			}
			
			
			//$offset,$offset+$limit-1,
            //WHERE P_ROWID >=%d AND P_ROWID <=%d  
			//WHERE P_ROWID >=3 AND P_ROWID <= 4 
			...
			*/
			...
			$rt .= ' ' . $limit;
			*/
			return array('limitpre'=>$pre,'limitrt'=>$rt,'limitrn'=>$rn);
		}
		return null;
	}
/**
 ...
 */
    ...
    function renderStatement($type, $data) {
		...
		switch (strtolower($type)) {
			case 'select':
				return "{$limit['limitpre']} SELECT {$fields} {$limit['limitrn']} FROM {$table} {$alias} {$joins} {$conditions} {$group} {$order} {$limit['limitrt']}";
			break;
			case 'create':
    ...
    function saveBlob(){
	    ...
	    $statementId = db2_prepare($this->connection, $sql);
		if (!$statementId) {
			return false;
		}

		db2_bind_param($statementId, 1, 'data',DB2_PARAM_INOUT,DB2_BINARY);
		
		//执行
		return db2_execute($statementId);
	}
    function saveData(){
		$args = func_get_args();
		if (!$args){
	...
	function getData(){
		...
		$state = db2_execute($statementId);
		if ($state){
			$Data = array();
			while ($dataInfo = db2_fetch_assoc($statementId)){
				$Data[] = $dataInfo;
			}
