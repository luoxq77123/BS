 ...
 */
	function value($data, $column = null, $safe = false) {
		...
		switch($column) {
			...
			break;
			case 'timestamp':
				$data = "to_timestamp('$data','yyyy-mm-dd hh24:mi:ss.ff')";
			break;
			case 'integer' :
			case 'float' :
			case null :
 ...
 */
	...
	function saveBlob(){
		...
		$data = $args[1];
		//$field = $args[2];
	    //$statementId = @ociparse($this->connection, $sql);
	    $statementId = oci_parse($this->connection, $sql);
		if (!$statementId) {
			$this->_setError($this->connection);
			return false;
		}
		...
		oci_bind_by_name($statementId, ":data", $blob,-1,OCI_B_BLOB);

		$blob->writetemporary($data, OCI_TEMP_BLOB);
		
		//执行
		$state = oci_execute($statementId);
	    if (!$state) {
			$this->_setError($statementId);
		}
		//$state = oci_execute($statementId);
	...
	function getData(){
	    ...
	    if (!$state) {
			...
			$Data = array();
			while ($dataInfo = oci_fetch_array($statementId,OCI_ASSOC)){
				$Data[] = $dataInfo;
			}
			oci_free_statement($statementId);
			return 	$Data;
